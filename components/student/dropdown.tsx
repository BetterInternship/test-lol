 ...
 * @ Modified time: 2025-06-15 01:03:57
 * @ Description:
 * 
...
export const GroupableRadioDropdown = ({ 
	name, options, on_change, default_value = options[0],
}: { 
	name: string, 
	options: string[],
	...
	const { active_dropdown, set_active_dropdown } = useContext(DropdownGroupContext);
	...
	return (
      ...
      >
        <span className={`${is_mobile ? 'text-sm' : 'text-sm'} whitespace-nowrap truncate`}>{value}</span>
        <ChevronDown className={`w-4 h-4 transition-transform flex-shrink-0 text-gray-400 ml-1 ${is_open ? 'rotate-180' : ''}`} />
      </Button>
        ...
        <div className={`absolute top-full mt-2 bg-white rounded-2xl shadow-xl z-50 min-w-[200px] overflow-hidden border border-gray-100`}>
          {options.map((option, index) => (
            <button
              ...
              className={`w-full text-left px-4 ${is_mobile ? 'py-3 text-sm font-medium' : 'py-2 text-sm'} hover:bg-gray-50 transition-colors duration-150 ${
                index === 0 ? '' : ''
              } ${
