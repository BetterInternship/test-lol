    ...
    return (
        <Dialog open={isOpen} onOpenChange={onClose}>
            <DialogContent className="max-w-4xl w-full h-[75vh] flex flex-col p-6">
                <DialogHeader>
                    <DialogTitle className="text-2xl font-bold">Document History for {student.name}</DialogTitle>
                    <DialogDescription>
                        Overview of all documents and their current status for {student.name}.
                    </DialogDescription>
                </DialogHeader>
                        ...
                        <TableBody>
                            {student.documents.map((doc, index) => (
                                <TableRow key={index}>
                                    <TableCell className="font-medium">
                                        {doc.pdfUrl ? (
                                            <a
                                                ...
                                                onClick={(e) => {
                                                    e.preventDefault();
                                                    handleDownload(doc.pdfUrl!, doc.prefix + doc.baseName + ".pdf");
                                                }}
                                    ...
                                    <TableCell className={cn("flex items-center gap-2", getStatusColorClass(doc.status))}>
